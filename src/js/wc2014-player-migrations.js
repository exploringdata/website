...
function draw_sankey() {
    ...
    // Keep layout info on graph.nodes/links for use in click handlers
    ...
    node.append('rect')
        ...
        .style('fill', d => { d.color = color(node_name(d.name)); return d.color; })
...
function update_links(links) {
    ...
    svg.selectAll('path.link')
        .style('opacity', d => keys.has(d.key) ? 1 : 0);
}
...
function show_players(players) {
    ...
    container.selectAll('div').data(players).enter().append('div')
        .attr('class', 'row')
        .html(d =>
            '<div class="col-md-2"><img class="img-thumbnail" src="' + d.image + '"/></div>' +
            '<div class="player-info col-md-10"><h5>' + d.firstName + ' ' + d.lastName + ': ' + d.birthCountry + sep.link + d.nationality + '</h5></div>'
        );
}
...
function link_click(event, d) {
    ...
    const players = data.filter(rec =>
        node_name(d.source.name) === rec.birthCountry &&
        node_name(d.target.name) === rec.nationality
    );
    show_players(players);
}
