...
function draw_sankey() {
    ...
    // d3-sankey mutates nodes/links in place
    ...
    node.append('rect')
        ...
        .style('fill', d => { d.color = color(team_name(d.name)); return d.color; })
...
function summary() {
    ...
    // convert teams into a sorted (desc) array
    graph.teams = Object.entries(graph.teams)
    ...
    // player stats
    graph.players = Object.entries(graph.players)
        .map(([key, value]) => ({ key, value }))
        .sort((a, b) => b.value - a.value);
